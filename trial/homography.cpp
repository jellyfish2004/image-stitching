...
int ransac_homography(const std::vector<Keypoint>& kp1,
    ...
    // Parallelized outer RANSAC loop.
    // Each thread maintains its own best (local_best_inliers, local_best_H)
    // and its own LCG RNG state seeded differently per thread,
    // avoiding any shared mutable state during iteration.
    // A critical section merges thread-local bests at the end.
    #pragma omp parallel
    {
        int local_best_inliers = 0;
        Mat3 local_best_H;

        // Per-thread independent RNG seed derived from thread id
        unsigned int rng_state = 42u + static_cast<unsigned int>(omp_get_thread_num()) * 1000003u;

        // Lambda-style inline RNG matching original LCG parameters
        auto rand_int_local = [&](int max) -> int {
            rng_state = rng_state * 1664525u + 1013904223u;
            return static_cast<int>((rng_state >> 1) % static_cast<unsigned>(max));
        };
        ...
        for (int iter = 0; iter < iterations; ++iter) {
            ...
            // Parallelized inlier counting with reduction over matches
            int inliers = 0;
            #pragma omp simd reduction(+:inliers)
            for (int m = 0; m < nm; ++m) {
            ...
            if (inliers > local_best_inliers) {
                local_best_inliers = inliers;
                local_best_H = H;
            }
        }

        // Merge thread-local bests into global best
        #pragma omp critical
        {
            if (local_best_inliers > best_inliers) {
                best_inliers = local_best_inliers;
                best_H = local_best_H;
            }
        }
    }
