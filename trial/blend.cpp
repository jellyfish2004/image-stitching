...
Image blend_simple(const Image& warped1,
    ...
    // Each (x, y) maps to a unique index in acc and wsum — no conflicts
    ...
    // Each (x, y) maps to a unique (cx, cy) via direct offset — no conflicts
    ...
    // Each index i is fully independent — no dependencies across iterations
    #pragma omp parallel for schedule(static)
